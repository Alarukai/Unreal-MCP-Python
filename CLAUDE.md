...
An MCP (Model Context Protocol) server for Unreal Engine — 189 tools across 23 subsystems. TypeScript/Node.js, ESM, communicates with UE via four transport layers.
...
stdio (MCP protocol)
  ...
  → 23 Tool Modules           # Each exports register*Tools(server, manager, config)
...
Our differentiator: 189 tools, 4 transport layers (most projects have 1), graceful degradation, no mandatory C++ plugin.

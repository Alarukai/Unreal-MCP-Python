...
An MCP (Model Context Protocol) server for Unreal Engine — 197 tools across 24 subsystems. TypeScript/Node.js, ESM, communicates with UE via four transport layers.
...
stdio (MCP protocol)
  ...
  → 24 Tool Modules           # Each exports register*Tools(server, manager, config)
...
Our differentiator: 197 tools, 4 transport layers (most projects have 1), graceful degradation, no mandatory C++ plugin.

...
An MCP (Model Context Protocol) server for Unreal Engine — 210 tools across 25 subsystems. TypeScript/Node.js, ESM, communicates with UE via four transport layers.
...
stdio (MCP protocol)
  ...
  → 25 Tool Modules           # Each exports register*Tools(server, manager, config)
...
Our differentiator: 210 tools, 4 transport layers (most projects have 1), graceful degradation, no mandatory C++ plugin.
